...
CREATE TABLE IF NOT EXISTS projects (
    ...
    start_date DATE,
    end_date DATE,
    boq_approved BOOLEAN DEFAULT FALSE,
    boq_approved_by VARCHAR(100),
    boq_approved_at TIMESTAMP WITH TIME ZONE,
...
CREATE TABLE IF NOT EXISTS boq_items (
    ...
    client_boq_sub_section VARCHAR(100),
    zoho_item_id VARCHAR(100),
    is_manpower_cost BOOLEAN DEFAULT FALSE,
    sort_order INT DEFAULT 0,
    frozen_qty NUMERIC(15, 3),
    frozen_rate NUMERIC(15, 3),
...
CREATE TABLE IF NOT EXISTS suppliers (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(255) NOT NULL,
    type VARCHAR(50) CHECK (type IN ('Material Supplier', 'Subcontractor', 'Both')),
...
CREATE TABLE IF NOT EXISTS purchase_orders (
    ...
    discount_before_vat NUMERIC(15, 3) DEFAULT 0.000, -- Discount applied before VAT calculation
    discount_before_vat_type VARCHAR(20) DEFAULT 'amount' CHECK (discount_before_vat_type IN ('amount', 'percentage')),
    discount_after_vat NUMERIC(15, 3) DEFAULT 0.000, -- Discount deducted after VAT calculation
    discount_after_vat_type VARCHAR(20) DEFAULT 'amount' CHECK (discount_after_vat_type IN ('amount', 'percentage')),
    terms_and_conditions TEXT,
    creation_mode VARCHAR(20) DEFAULT 'item' CHECK (creation_mode IN ('item', 'section')),
    zoho_po_id VARCHAR(100),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS grn_lines (
    ...
    amount_received NUMERIC(15, 3) DEFAULT 0.000,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS client_boq_items (
    ...
    unit_rate NUMERIC(15, 3) DEFAULT 0.000,
    sort_order INT DEFAULT 0,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
-- 19. Activity Logs for Project Audit and Change Tracking
...
-- 21. Internal Manpower Client Links (Junction table)
