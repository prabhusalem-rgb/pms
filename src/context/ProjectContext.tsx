...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshProjects = useCallback(() => {
    ...
    // Only update projects state if the data actually changed (avoid re-render churn)
    setProjects(prev => {
      if (
        prev.length === list.length &&
        prev.every((p, i) => p.id === list[i]?.id && p.name === list[i]?.name)
      ) {
    ...
    if (list.length > 0) {
      ...
      // No else — don't call setActiveProject unnecessarily when ID hasn't changed
    } else {
  ...
  const contextValue = useMemo(() => ({
    ...
    syncStatus,
    syncData,
  }), [projects, activeProjectId, activeProject, setActiveProjectId, refreshProjects, isSyncing, syncStatus, syncData]);
