...
export interface PurchaseOrder {
  ...
  discount_before_vat_type?: 'amount' | 'percentage';
  discount_after_vat?: number;
  discount_after_vat_type?: 'amount' | 'percentage';
  terms_and_conditions?: string;
  created_at: string;
  zoho_po_id?: string | null;
  creation_mode?: 'item' | 'section';
}
...
export interface SectionBudgetSummary {
  ...
  sub_section: string;
  key: string;
  boq_budget_excl: number;
  boq_vat: number;
  boq_budget_incl: number;
  po_ordered_excl: number;
  po_ordered_vat: number;
  po_ordered_incl: number;
  remaining_budget_excl: number;
  remaining_budget_incl: number;
...
class DatabaseManager {
  ...
  saveMaterialIssue(
    ...
    this.logUserAction(`Created new Material Issued Note (MIN) "${newIssue.issue_number}"`);
  ...
  deleteGRN(grnId: string): void {
    ...
    if (receivedBOQItemIds.size > 0) {
      ...
      if (hasMINBooked) {
        throw new Error(`This Goods Receipt Note (GRN) "${grn.grn_number}" cannot be deleted because the received materials have already been booked/issued under one or more Material Issued Notes (MIN). Please delete the associated MINs first.`);
      }
    }
  ...
  deleteMaterialIssue(issueId: string): void {
    ...
    if (deletedIssue) {
      this.logUserAction(`Deleted Material Issued Note (MIN) "${deletedIssue.issue_number}"`);
    }
  }
  ...
  // --- Section & Sub-Section Budget & Procurement Summary ---
  getSectionBudgetSummary(projectId: string): SectionBudgetSummary[] {
    const boqItems = this.getBOQItems(projectId);
    const pos = this.getPOs(projectId);
    const activePOs = pos.filter(po => po.status !== 'draft');
    const allLines = this.get<PurchaseOrderLine>('po_lines', []);
    ...
    // 1. Group BOQ items by Section & Sub-Section
    ...
    boqItems.forEach(b => {
      const sec = (b.section || 'General Works').trim();
      const sub = (b.sub_section || 'General').trim();
      const key = `${sec}:::${sub}`;
      ...
      entry.itemIds.add(b.id);
      const excl = (b.approved_qty || 0) * (b.unit_rate || 0);
      const vat = excl * ((b.vat_rate || 5.0) / 100);
      entry.boq_budget_excl += excl;
      entry.boq_vat += vat;
      entry.boq_budget_incl += (excl + vat);
    });

    // Also include any section/sub-section that exists in PO lines even if not in BOQ
    activePOs.forEach(po => {
      const poLines = allLines.filter(l => l.po_id === po.id);
      poLines.forEach(l => {
        if (!l.boq_item_id && (l.section || l.sub_section)) {
          const sec = (l.section || 'General Works').trim();
          const sub = (l.sub_section || 'General').trim();
          const key = `${sec}:::${sub}`;
          if (!sectionMap.has(key)) {
            sectionMap.set(key, {
              section: sec,
    ...
    // 2. Aggregate PO commitments per Section & Sub-Section
    const results: SectionBudgetSummary[] = [];

    sectionMap.forEach((entry, key) => {
      let poOrderedExcl = 0;
      let poOrderedVat = 0;
      ...
      activePOs.forEach(po => {
        ...
        let poSectionLinesExcl = 0;
        let poSectionLinesVat = 0;
        let hasSectionLine = false;
        ...
        poLines.forEach(l => {
          ...
          if (l.boq_item_id) {
            // Item-based PO line: matches if boq_item_id belongs to this section & sub_section
            if (entry.itemIds.has(l.boq_item_id)) {
              matches = true;
            }
          } else if (l.section || l.sub_section) {
            // Section-based PO line: matches if section and sub_section match
            const lSec = (l.section || 'General Works').trim();
            const lSub = (l.sub_section || 'General').trim();
            if (lSec === entry.section && lSub === entry.sub_section) {
              matches = true;
              isSecMode = true;
            }
          }
          ...
          if (matches) {
            const lineExcl = (l.qty || 0) * (l.unit_rate || 0);
            const lineVat = lineExcl * ((l.vat_rate || 5.0) / 100);
            poSectionLinesExcl += lineExcl;
            poSectionLinesVat += lineVat;
            if (isSecMode || po.creation_mode === 'section') {
              hasSectionLine = true;
            }
          }
        });

        if (poSectionLinesExcl > 0 || poSectionLinesVat > 0) {
          poOrderedExcl += poSectionLinesExcl;
          poOrderedVat += poSectionLinesVat;
          ...
          poMap.set(po.id, {
            ...
            amount_excl: poSectionLinesExcl,
            amount_incl: poSectionLinesExcl + poSectionLinesVat,
            is_section_mode: hasSectionLine || po.creation_mode === 'section'
          });
        }
      });

      const poOrderedIncl = poOrderedExcl + poOrderedVat;
      const remainingExcl = Math.max(0, entry.boq_budget_excl - poOrderedExcl);
      const remainingIncl = Math.max(0, entry.boq_budget_incl - poOrderedIncl);
      const utilization = entry.boq_budget_excl > 0 ? (poOrderedExcl / entry.boq_budget_excl) * 100 : (poOrderedExcl > 0 ? 100 : 0);

      results.push({
        section: entry.section,
        sub_section: entry.sub_section,
        key,
        boq_budget_excl: entry.boq_budget_excl,
        boq_vat: entry.boq_vat,
        boq_budget_incl: entry.boq_budget_incl,
        po_ordered_excl: poOrderedExcl,
        po_ordered_vat: poOrderedVat,
        po_ordered_incl: poOrderedIncl,
        remaining_budget_excl: remainingExcl,
        remaining_budget_incl: remainingIncl,
        utilization_percent: utilization,
        po_count: poMap.size,
        po_list: Array.from(poMap.values())
      });
    });

    // Sort by Section then Sub-Section
