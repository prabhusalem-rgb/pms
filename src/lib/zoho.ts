...
class ZohoBooksClient {
  ...
  async syncPurchaseOrder(poId: string): Promise<string> {
    ...
    const isPercent = po.discount_before_vat_type === 'percentage';
    const discountVal = po.discount_before_vat ? Number(po.discount_before_vat) : 0;

    const payload: any = {
      vendor_id: zohoContactId,
      date: new Date(po.created_at).toISOString().split('T')[0],
      line_items: syncedLineItems,
      discount: isPercent ? `${discountVal}%` : discountVal,
      discount_type: isPercent ? 'entity_level' : 'entity_level',
