...
export default function ProcurementPage() {
  ...
  const [poLines, setPoLines] = useState<Array<{
    ...
    [{ boq_item_id: null, is_non_boq: false, searchText: '', description: '', unit: '', qty: 0, unit_rate: 0, vat_rate: 5.0 }]
  );
  ...
  const handleCreateClick = () => {
    ...
    setPoLines([{ boq_item_id: null, is_non_boq: false, searchText: '', description: '', unit: '', qty: 0, unit_rate: 0, vat_rate: 5.0 }]);
    setPoType('material');
  ...
  const handleEditClick = (po: PurchaseOrder) => {
    ...
    const lines = db.getPOLines(po.id).map(l => {
      ...
      return {
        ...
        unit: boq ? boq.unit : (l as any).unit || '',
        qty: l.qty,
        unit_rate: l.unit_rate,
        vat_rate: l.vat_rate
      };
    });
    setPoLines(lines.length > 0 ? lines : [{ boq_item_id: null, is_non_boq: false, searchText: '', description: '', unit: '', qty: 0, unit_rate: 0, vat_rate: 5.0 }]);
    setSupplierSearchQuery('');
    setShowSupplierDropdown(false);
    setShowModal(true);
  };
  ...
  const handleLineChange = (index: number, field: string, value: any) => {
    ...
    if (field === 'boq_item_id') {
      ...
      if (selectedBOQ) {
        ...
        updated[index].unit_rate = selectedBOQ.unit_rate;
  ...
  const addLine = () => {
    setPoLines(prev => [...prev, { boq_item_id: null, is_non_boq: false, searchText: '', description: '', unit: '', qty: 0, unit_rate: 0, vat_rate: 5.0 }]);
  };

  const addNonBOQLine = () => {
    setPoLines(prev => [...prev, { boq_item_id: null, is_non_boq: true, searchText: '', description: '', unit: '', qty: 0, unit_rate: 0, vat_rate: 5.0 }]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validate duplicate BOQ Items in the PO
    const seenBOQItems = new Set<string>();
    for (const line of poLines) {
      if (line.boq_item_id) {
        if (seenBOQItems.has(line.boq_item_id)) {
          const matchedItem = boqItems.find(b => b.id === line.boq_item_id);
          alert(`Error: Duplicate item detected: "${matchedItem?.description || 'Item'}" is added multiple times. Please combine duplicate rows into a single item row with cumulative quantities.`);
          return;
        }
    ...
    setPoLines([{ boq_item_id: null, is_non_boq: false, description: '', unit: '', qty: 0, unit_rate: 0, vat_rate: 5.0 }]);
    setEditingPoId(null);
    setPoStatus('draft');
    setShowModal(false);
    loadData();
      ...
      {showModal && (
        <div className="overlay">
          <div className="modal-content modal-lg">
            ...
            <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 320px) 1fr', gap: '2rem', alignItems: 'stretch' }}>
              ...
              <div style={{ background: 'var(--bg-main)', padding: '1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div className="form-group" style={{ marginBottom: 0 }}>
                  ...
                  >
                    ...
                    <option value="subcontract">Work Order</option>
                  </select>
                </div>
                    ...
                    {showSupplierDropdown && (
                      <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                          ...
                          {suppliers
                            .filter(s => poType === 'subcontract' ? s.type === 'Subcontractor' : s.type === 'Material Supplier')
                            .filter(s => 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div style={{ fontSize: '0.8rem', lineHeight: '1.2' }}>
                                      <strong>{b.item_code}</strong> - {b.description} <span style={{ color: 'var(--text-muted)', fontSize: '0.72rem' }}>(Rem: {b.boq_balance})</span>
                                    </div>
                                  </label>
                                );
                              })
                  ...
                  <div style={{ overflowX: 'auto', maxHeight: '380px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-sm)', marginBottom: '1rem' }}>
                <table className="table line-items-table" style={{ fontSize: '0.85rem', width: '100%', minWidth: '850px' }}>
                    ...
                    {poLines.map((line, idx) => {
                      ...
                      return (
                        <tr key={idx} style={{ backgroundColor: isNonBOQ ? 'rgba(245, 158, 11, 0.04)' : 'transparent' }}>
                                  ...
                                  {boqItems
                                    ...
                                    .map(b => (
                                      <option key={b.id} value={`${b.item_code} - ${b.description}`}>
                                        Remaining: {b.boq_balance} {b.unit}
                                      </option>
                                    ))
                                  }
                                </datalist>
                          ...
                          {/* Description column (auto for BOQ, empty for non-BOQ since it's the main col) */}
                          <td style={{ padding: '0.5rem 0.75rem' }}>
                            {!isNonBOQ ? (
                              <>
                                <div style={{ fontSize: '0.8rem', fontWeight: '500', minWidth: '120px', whiteSpace: 'normal', wordBreak: 'break-word', color: 'var(--text-main)' }}>
                                  {line.description || <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Auto-filled from BOQ</span>}
                                </div>
                                {(() => {
                                  const bItem = boqItems.find(b => b.id === line.boq_item_id);
                                  if (!bItem) return null;
                                  return (
                                    <div style={{ fontSize: '0.72rem', color: 'var(--primary-hover)', marginTop: '0.2rem', fontWeight: '600' }}>
                                      App: {bItem.approved_qty} | Ord: {bItem.ordered_qty} | Rem: {bItem.boq_balance}
                                    </div>
                                  );
                                })()}
                              </>
                            ) : (
                              <span style={{ fontSize: '0.72rem', color: 'var(--accent)', fontWeight: '600', padding: '0.2rem 0.4rem', background: 'rgba(245,158,11,0.1)', borderRadius: '4px' }}>Non-BOQ</span>
                            )}
                          </td>
                          <td style={{ padding: '0.5rem 0.75rem' }}>
                            {!isNonBOQ ? (
                              <span style={{ fontSize: '0.85rem', fontWeight: '500', color: 'var(--text-main)' }}>
                                {(() => {
                                  const b = boqItems.find(item => item.id === line.boq_item_id);
                                  return b ? b.unit : 'N/A';
                                })()}
                              </span>
                            ) : (
                          ...
                          <td style={{ padding: '0.5rem 0.75rem' }}>
                            <input
                              ...
                              max={(() => {
                                const bItem = boqItems.find(b => b.id === line.boq_item_id);
                                return bItem ? bItem.boq_balance : undefined;
                              })()}
                            ...
                            {(() => {
                              const bItem = boqItems.find(b => b.id === line.boq_item_id);
                              if (bItem && line.qty > bItem.boq_balance) {
                                return (
                          ...
                          <td style={{ padding: '0.5rem 0.75rem' }}>
                            ...
                            {(() => {
                              ...
                              if (!priceInfo) return <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>Last: N/A</div>;
                              
                              if (priceInfo.isOtherProject) {
                                return (
                                  <div style={{ fontSize: '0.65rem', color: 'var(--accent)', marginTop: '0.2rem', fontWeight: '700', lineHeight: '1.2' }} title={`From Project: ${priceInfo.projectName}`}>
                                    Last (Other Proj):<br/>OMR {priceInfo.price.toFixed(3)}
                                  </div>
                                );
                              }
                              return (
                                <div style={{ fontSize: '0.68rem', color: 'var(--secondary)', marginTop: '0.2rem', fontWeight: '600' }}>
            ...
            <div className="modal-content" style={{ maxWidth: '900px', width: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column', padding: '2rem' }}>
              ...
              <div className="modal-header print-hide" style={{ padding: '0 0 0.5rem 0', borderBottom: '1px solid var(--border-color)', marginBottom: '0.75rem' }}>
                <h2 className="modal-title" style={{ fontSize: '1.1rem' }}>
                  📄 {printPo.type === 'subcontract' ? 'Work Order' : 'Purchase Order'} Preview
                </h2>
              ...
              <div className="print-po-area" style={{ flex: 1, overflowY: 'auto', paddingRight: '0.5rem', fontFamily: 'Arial, sans-serif' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '0.75rem' }}>
                  <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 'bold', textDecoration: 'underline', color: '#0f172a', letterSpacing: '0.5px' }}>
                    {printPo.type === 'subcontract' ? 'WORK ORDER' : 'PURCHASE ORDER'}
                  </h2>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '0.75rem', fontSize: '0.75rem', lineHeight: '1.4' }}>
                  <div style={{ border: '1px solid #e2e8f0', padding: '0.5rem', borderRadius: '4px' }}>
                    <div style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '0.2rem', marginBottom: '0.4rem', fontWeight: 'bold', color: '#475569' }}>
                      {printPo.type === 'subcontract' ? 'WORK ORDER DETAILS' : 'ORDER DETAILS'}
                    </div>
                      ...
                      printLines.map((line, idx) => {
                        ...
                        return (
                          <tr key={line.id} style={{ borderBottom: '1px solid #cbd5e1' }}>
                            ...
                            <td style={{ border: '1px solid #cbd5e1', padding: '0.3rem 0.4rem', textAlign: 'right' }}>{line.qty}</td>
