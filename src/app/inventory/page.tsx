...
export default function InventoryPage() {
  ...
  const handleIssueSubmit = (e: React.FormEvent) => {
    ...
    if (issueDuplicate) {
      alert(`Error: A Material Issued Note (MIN) with number "${issueHeader.issue_number}" already exists. Please enter a unique MIN number.`);
      return;
    }
  ...
  const handleDeleteIssue = (id: string, issueNumber: string) => {
    if (window.confirm(`Are you sure you want to delete Material Issued Note "${issueNumber}"? This will return the issued quantities back to the stock ledger.`)) {
      if (window.confirm(`⚠️ WARNING: This action is permanent and cannot be undone. Are you absolutely sure you want to proceed with deleting Material Issued Note "${issueNumber}"?`)) {
    ...
    <div>
      ...
      {activeTab === 'grn' && (
       ...
       {activeTab === 'issue' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', marginTop: '0.5rem' }}>
            <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>Material Issued Notes (MIN) history</span>
              ...
              {issues.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>
                    No Material Issued Notes recorded yet.
                  </td>
                </tr>
              ) : (
                issues.map((i) => {
      ...
      {showGRNModal && (
        <div className="overlay">
          <div className="modal-content modal-lg" style={{ maxWidth: '1050px', width: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <form onSubmit={handleGRNSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
              <div style={{ overflowY: 'auto', flex: 1, paddingRight: '0.5rem', marginBottom: '0.5rem' }}>
                        ...
                        <table className="table" style={{ fontSize: '0.8rem' }}>
                          <thead>
                            <tr style={{ backgroundColor: '#f8fafc' }}>
                              ...
                              <th style={{ textAlign: 'right', width: '220px' }}>Receive Now (Qty or OMR Value)</th>
                            </tr>
                          </thead>
                          <tbody>
                            {poLines.map((line) => {
                                ...
                                <tr key={line.id}>
                                  ...
                                  <td style={{ textAlign: 'right' }}>
                                    <input
                                      ...
                                      style={{ width: '100%', minWidth: '180px', textAlign: 'right' }}
                                      required
                                    />
                                  </td>
                                </tr>
      ...
      {showIssueModal && (
        <div className="overlay">
          <div className="modal-content modal-lg" style={{ maxHeight: '85vh', padding: '2rem' }}>
            <div className="modal-header">
              <h2 className="modal-title">Record Material Issued Note (MIN)</h2>
