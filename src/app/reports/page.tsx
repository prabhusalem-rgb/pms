...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'financial' | 'section-pos' | 'stock' | 'wastage' | 'received' | 'cost-variation'>('financial');
  ...
  const calculateReport = () => {
    if (activeProjectId) {
      ...
      const sectionSummaries = db.getSectionBudgetSummary(activeProjectId);
  ...
  const exportSectionPOReport = () => {
    const dataToExport = sectionData.map(item => {
      const boqVal = vatView === 'inclusive' ? item.boq_budget_incl : item.boq_budget_excl;
      const poVal = vatView === 'inclusive' ? item.po_ordered_incl : item.po_ordered_excl;
      const remVal = vatView === 'inclusive' ? item.remaining_budget_incl : item.remaining_budget_excl;
      return {
        ...
        remaining: remVal,
        utilization: (item.utilization_percent / 100),
        status: item.utilization_percent >= 100 ? 'Fully Committed' : (item.utilization_percent >= 80 ? 'Near Limit' : 'Under Budget')
      };
    });

    exportToExcel(
      `Section_PO_Report_${activeProject?.name || 'Project'}.xls`,
      'Section PO Summary',
      [
        { header: 'Section', key: 'section', width: 25 },
        { header: 'Sub-section', key: 'sub_section', width: 25 },
        { header: `BOQ Authorized Budget (${vatView === 'inclusive' ? 'Incl.' : 'Excl.'} VAT)`, key: 'boq_budget', width: 22, type: 'currency' },
        { header: `Total POs Issued (${vatView === 'inclusive' ? 'Incl.' : 'Excl.'} VAT)`, key: 'po_ordered', width: 22, type: 'currency' },
        { header: 'PO Count', key: 'po_count', width: 12, type: 'number' },
        { header: `Remaining Balance (${vatView === 'inclusive' ? 'Incl.' : 'Excl.'} VAT)`, key: 'remaining', width: 22, type: 'currency' },
        { header: 'Budget Utilization (%)', key: 'utilization', width: 20, type: 'percent' },
  ...
  return (
    <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {asOfDate && (
              <button
                ...
                onClick={() => setAsOfDate('')}
                style={{ padding: '0.3rem 0.5rem', fontSize: '0.75rem', height: 'auto', minHeight: 'unset' }}
              >
                Clear
              </button>
            )}
            ...
            <div style={{ display: 'flex', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
              <button
                ...
                style={{
                  ...
                  fontWeight: '600',
                  border: 'none',
                  background: vatView === 'exclusive' ? 'var(--primary)' : 'var(--card-bg)',
                  color: vatView === 'exclusive' ? '#fff' : 'var(--text-muted)',
                  cursor: 'pointer'
                }}
              >
                Excl. 5% VAT
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: '600',
                  border: 'none',
                  background: vatView === 'inclusive' ? 'var(--primary)' : 'var(--card-bg)',
                  color: vatView === 'inclusive' ? '#fff' : 'var(--text-muted)',
                  cursor: 'pointer'
                }}
              >
                Incl. 5% VAT
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        ...
        <button
          onClick={() => setActiveTab('section-pos')}
          style={{
            padding: '0.6rem 1.2rem',
            border: 'none',
            background: 'none',
            borderBottom: activeTab === 'section-pos' ? '3px solid var(--primary)' : '3px solid transparent',
            color: activeTab === 'section-pos' ? 'var(--primary)' : 'var(--text-muted)',
            fontWeight: '700',
            cursor: 'pointer',
            fontSize: '0.88rem'
          }}
        >
          📑 Section / Sub-section PO Report
        </button>
        <button
      ...
      {activeTab === 'section-pos' && (() => {
        const filtered = sectionData.filter(item =>
          item.section.toLowerCase().includes(searchQuery.toLowerCase()) ||
          item.sub_section.toLowerCase().includes(searchQuery.toLowerCase())
        );

        const totalBoqBudget = sectionData.reduce((sum, item) => sum + (vatView === 'inclusive' ? item.boq_budget_incl : item.boq_budget_excl), 0);
        const totalPoOrdered = sectionData.reduce((sum, item) => sum + (vatView === 'inclusive' ? item.po_ordered_incl : item.po_ordered_excl), 0);
        const totalRemaining = Math.max(0, totalBoqBudget - totalPoOrdered);
        const overallUtilization = totalBoqBudget > 0 ? (totalPoOrdered / totalBoqBudget) * 100 : 0;
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
              <div className="card" style={{ padding: '1rem', borderLeft: '4px solid var(--primary)' }}>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: '700' }}>
                  Total BOQ Authorized
                </div>
                <div style={{ fontSize: '1.25rem', fontWeight: '800', marginTop: '0.25rem' }}>
                  OMR {totalBoqBudget.toLocaleString('en-US', { minimumFractionDigits: 3 })}
                </div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
                  Across all sections & sub-sections
                </div>
              </div>
              ...
              <div className="card" style={{ padding: '1rem', borderLeft: '4px solid #6366f1' }}>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: '800', marginTop: '0.25rem', color: '#6366f1' }}>
                  OMR {totalPoOrdered.toLocaleString('en-US', { minimumFractionDigits: 3 })}
                </div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
                  Item-based & section-based orders
                </div>
              </div>
              ...
              <div className="card" style={{ padding: '1rem', borderLeft: '4px solid var(--success)' }}>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: '800', marginTop: '0.25rem', color: 'var(--success)' }}>
                  OMR {totalRemaining.toLocaleString('en-US', { minimumFractionDigits: 3 })}
                </div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
                  Available purchasing balance
                </div>
              </div>
              ...
              <div className="card" style={{ padding: '1rem', borderLeft: '4px solid var(--accent)' }}>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: '800', marginTop: '0.25rem', color: overallUtilization > 95 ? 'var(--danger)' : 'var(--accent)' }}>
            ...
            <div className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '1rem' }}>
                <div>
                  <h2 style={{ fontSize: '1.1rem', fontWeight: '700', margin: 0 }}>
                    📑 Purchase Orders by Section & Sub-section Breakdown
                  </h2>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    Compares Cost Sheet BOQ Section budgets against all issued Purchase Orders (by Item & by Section)
                  </span>
                </div>

                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <button onClick={exportSectionPOReport} className="btn btn-outline" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.82rem', padding: '0.4rem 0.8rem', height: 'auto', minHeight: 'unset' }}>
                    📥 Export to Excel
                  </button>
                  <input
                    type="text"
                    placeholder="🔍 Filter by section or sub-section..."
                    value={searchQuery}
              ...
              <div className="table-container" style={{ overflowX: 'auto' }}>
                <table className="table table-compact">
                  <thead>
                    <tr>
                      <th style={{ minWidth: '180px' }}>Section</th>
                      <th style={{ minWidth: '160px' }}>Sub-section</th>
                      <th style={{ textAlign: 'right' }}>BOQ Budget (OMR)</th>
                      <th style={{ textAlign: 'right' }}>Total PO Issued (OMR)</th>
                      <th style={{ textAlign: 'center', width: '80px' }}>POs</th>
                      <th style={{ textAlign: 'right' }}>Remaining Balance (OMR)</th>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan={8} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>
                          {sectionData.length === 0
                            ? "No Section/Sub-section items found in BOQ."
                            : "No matching Sections or Sub-sections found."}
                        </td>
                      </tr>
                    ) : (
                      filtered.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx}>
                            <td style={{ fontWeight: '700', color: 'var(--text-main)' }}>{item.section}</td>
                            <td style={{ color: 'var(--text-main)' }}>{item.sub_section}</td>
                            <td style={{ textAlign: 'right', fontWeight: '600' }}>
                              {boqVal.toLocaleString('en-US', { minimumFractionDigits: 3 })}
                            </td>
                            <td style={{ textAlign: 'right', fontWeight: '700', color: poVal > 0 ? '#4f46e5' : 'inherit' }}>
                              {poVal.toLocaleString('en-US', { minimumFractionDigits: 3 })}
                            </td>
                            <td style={{ textAlign: 'center' }}>
                              {item.po_count > 0 ? (
                                <span className="badge" style={{ backgroundColor: '#ede9fe', color: '#6d28d9', fontWeight: '700', fontSize: '0.75rem' }} title={item.po_list.map((p: any) => `${p.po_number}: OMR ${p.amount_excl.toFixed(3)}`).join('\n')}>
                                  {item.po_count} PO{item.po_count > 1 ? 's' : ''}
                                </span>
                              ) : (
                                <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>-</span>
                              )}
                            </td>
                            <td style={{ textAlign: 'right', fontWeight: '700', color: remVal > 0 ? 'var(--success)' : 'var(--danger)' }}>
                              {remVal.toLocaleString('en-US', { minimumFractionDigits: 3 })}
                            </td>
                            <td>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <div style={{ flex: 1, height: '7px', backgroundColor: '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}>
                                  <div style={{
                                    ...
                                    height: '100%',
                                    backgroundColor: util >= 100 ? 'var(--danger)' : (util >= 80 ? 'var(--accent)' : 'var(--primary)')
                                  }} />
                                </div>
